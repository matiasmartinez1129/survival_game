...
(() => {
  ...
  // ---- Load assets ----
  function loadImage(src){ return new Promise(res=>{ const i=new Image(); i.onload=()=>res(i); i.onerror=()=>{ const p=new Image(); p.onload=()=>res(p); p.src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA4AAAAOCAYAAAAfSC3RAAAALElEQVQ4T2NkYGD4z0AJwDiqgQkGEiCwA5LJwqgYkQwGg0E0aA1GQ0kAgBp0gBEuA1VmAAAAAElFTkSuQmCC'; }; i.src=src; }); }
  ...
  // ---- Player ----
  const player = { x:300,y:300,r:6, speed:60,sprint:1.6, hunger:100,warmth:100,energy:100,oxygen:100, wood:0,stone:0, rawMeat:0,rawFish:0,cookedMeat:0,cookedFish:0,
    spearOwned:false,spearEquipped:false, facing:{x:1,y:0}, dirName:'right', invuln:0, animFrame:0, animTimer:0 };

  const cam = {x:0,y:0};

  // ---- Input ----
  const keys = new Set();
  onkeydown = e => { const k=e.key.toLowerCase(); keys.add(k); if (['s','l','c','i','b','escape'].includes(k)) e.preventDefault(); if (k==='i') toggleInventory(); if (k==='b') toggleBuild(); if (k==='escape'){ closePanels(); } };
  ...
  btnPress(btnA, ()=>{ if (build.mode) tryBuild(); else tryAttack(); });
  ...
  btnPress(btnDoor, ()=>{ /* placeholder for doors in this mobile pack */ });
  btnPress(btnMis, ()=>{ /* placeholder for missions in this mobile pack */ });
  btnFS.addEventListener('click', async ()=>{ try{ await document.body.requestFullscreen(); }catch{} });

  // ---- Panels ----
  function closePanels(){ document.getElementById('inventory').classList.add('hidden'); document.getElementById('buildPanel').classList.add('hidden'); }
  ...
  function updateInvUI(){ document.getElementById('woodCount').textContent=player.wood; document.getElementById('stoneCount').textContent=player.stone; document.getElementById('rawMeat').textContent=player.rawMeat; document.getElementById('rawFish').textContent=player.rawFish; document.getElementById('cookedMeat').textContent=player.cookedMeat; document.getElementById('cookedFish').textContent=player.cookedFish; document.getElementById('spearOwned').textContent=player.spearOwned?(player.spearEquipped?'equipada':'sí'):'no'; }
  document.getElementById('btnEquipSpear').onclick = ()=>{ if (!player.spearOwned) return toast('Primero crafteá la lanza.'); player.spearEquipped=!player.spearEquipped; updateInvUI(); };
  document.getElementById('btnCraftSpear').onclick = ()=>{ if (player.wood>=2 && player.stone>=1){ player.wood-=2; player.stone-=1; player.spearOwned=true; ASSETS.s_craft.play().catch(()=>{}); toast('Lanza creada'); updateInvUI(); } else toast('2 madera + 1 piedra'); };
  ...
  function start(){ requestAnimationFrame(tick); ASSETS.s_water.volume=0.0; ASSETS.s_water.play().catch(()=>{}); ASSETS.s_campfire.volume=0.0; ASSETS.s_campfire.play().catch(()=>{});
  ...
  function cook(which){ if (!nearCampfire()) return toast('Debes estar cerca de una fogata.'); if (which==='meat' && player.rawMeat>=1){ player.rawMeat--; player.cookedMeat++; ASSETS.s_craft.play().catch(()=>{}); toast('Carne cocinada'); updateInvUI(); } else if (which==='fish' && player.rawFish>=1){ player.rawFish--; player.cookedFish++; ASSETS.s_craft.play().catch(()=>{}); toast('Pescado cocinado'); updateInvUI(); } else toast('Faltan ingredientes'); }
  function tryAttack(){ if (!player.spearEquipped) return; const reach=18; for (let i=deers.length-1;i>=0;i--){ const d=deers[i]; if (Math.hypot(player.x+player.facing.x*reach-d.x, player.y+player.facing.y*reach-d.y)<player.r+d.r){ d.hp--; ASSETS.s_hit.currentTime=0; ASSETS.s_hit.play().catch(()=>{}); if (d.hp<=0){ deers.splice(i,1); player.rawMeat++; toast('Carne cruda +1'); updateInvUI(); } return; } } for (let i=fishs.length-1;i>=0;i--){ const f=fishs[i]; if (Math.hypot(player.x+player.facing.x*reach-f.x, player.y+player.facing.y*reach-f.y)<player.r+f.r){ fishs.splice(i,1); player.rawFish++; ASSETS.s_hit.play().catch(()=>{}); toast('Pescado crudo +1'); updateInvUI(); return; } } }
  function craftFire(){ if (!tick.craftLatch){ tick.craftLatch=true; if (player.wood>=3 && player.stone>=1){ player.wood-=3; player.stone-=1; const px=player.x+player.facing.x*10, py=player.y+player.facing.y*10; campfires.push({x:px,y:py,r:50,life:120}); ASSETS.s_craft.currentTime=0; ASSETS.s_craft.play().catch(()=>{}); toast('Fogata creada'); } else toast('3 madera + 1 piedra'); setTimeout(()=>{ tick.craftLatch=false; }, 200);} }
  function tryBuild(){ const gx=Math.round((player.x+player.facing.x*14)/16)*16; const gy=Math.round((player.y+player.facing.y*14)/16)*16; const type=build.current; if (type==='wall'){ if (player.wood<2) return toast('Pared: 2 madera'); player.wood-=2; } if (type==='floor'){ if (player.wood<1) return toast('Piso: 1 madera'); player.wood-=1; } if (type==='roof'){ if (player.wood<1||player.stone<1) return toast('Techo: 1 madera + 1 piedra'); player.wood-=1; player.stone-=1; } buildings.push({type,x:gx,y:gy,rot:build.rot%4}); ASSETS.s_craft.currentTime=0; ASSETS.s_craft.play().catch(()=>{}); updateInvUI(); }
  ...
  function tick(){
    ...
    const speed = player.speed * baseMod * ((keys.has('shift')||runTouch)?player.sprint:1) * (inWater?0.6:1.0);
    ...
    if (inWater) {
      ...
      if (under) { player.oxygen = Math.max(0, player.oxygen - 8*dt); if (player.oxygen===0) player.energy = Math.max(0, player.energy - 10*dt); }
    ...
    // click action
    if (mouseDown){ if (build.mode) tryBuild(); else tryAttack(); mouseDown=false; }

    // craft fire via C key (desktop)
    ...
    // time
    timeOfDay += (dt * 24 / DAY_SPEED); if (timeOfDay>=24){ timeOfDay-=24; day++; }
    shoreAnim += dt*4;
  ...
  function drawBuildGhost(){ if (!build.mode) return; const gx=Math.round((player.x + player.facing.x*14)/16)*16; const gy=Math.round((player.y + player.facing.y*14)/16)*16; const img=ASSETS[build.current]; if(!img) return; ctx.globalAlpha=0.6; ctx.drawImage(img, (Math.floor(gx - cam.x)-8)*SCALE, (Math.floor(gy - cam.y)-8)*SCALE, 16*SCALE, 16*SCALE); ctx.globalAlpha=1.0; }
  ...
  function render(inWater){
    ...
    for (const f of campfires){ const x=Math.floor(f.x - cam.x), y=Math.floor(f.y - cam.y); ctx.fillStyle='rgba(255,150,40,0.15)'; ctx.beginPath(); ctx.arc(x*SCALE, y*SCALE, 30*SCALE, 0, Math.PI*2); ctx.fill(); ctx.drawImage(ASSETS.campfire, (x-8)*SCALE, (y-8)*SCALE, 16*SCALE, 16*SCALE); }
    for (const b of buildings){ const x=Math.floor(b.x - cam.x), y=Math.floor(b.y - cam.y); const img=ASSETS[b.type]; if (!img) continue; ctx.drawImage(img, (x-8)*SCALE, (y-8)*SCALE, 16*SCALE, 16*SCALE); }
    ...
    if (nightAlpha>0){ ctx.save(); ctx.globalAlpha=nightAlpha; ctx.fillStyle='rgba(0,0,20,1)'; ctx.fillRect(0,0,cvs.width,cvs.height); ctx.globalCompositeOperation='destination-out'; radialHole(px*SCALE, py*SCALE, 55*SCALE); for (const f of campfires){ const x=Math.floor(f.x - cam.x), y=Math.floor(f.y - cam.y); radialHole(x*SCALE, y*SCALE, 110*SCALE); } ctx.restore(); ctx.globalCompositeOperation='source-over'; }
  }
  ...
  function drawMinimap(){ const mm=document.getElementById('minimap'); const mctx=mm.getContext('2d'); const mw=mm.width, mh=mm.height; mctx.fillStyle='#0b1320'; mctx.fillRect(0,0,mw,mh); mctx.fillStyle='#1e5fa7'; mctx.fillRect(water.x/WORLD.w*mw, water.y/WORLD.h*mh, water.w/WORLD.w*mw, water.h/WORLD.h*mh); mctx.fillStyle='#2c8a2e'; for (let i=0;i<trees.length;i+=4){ const t=trees[i]; const x=t.x/WORLD.w*mw, y=t.y/WORLD.h*mh; mctx.fillRect(x-1,y-1,2,2); } mctx.fillStyle='#ffa726'; for (const f of campfires){ const x=f.x/WORLD.w*mw, y=f.y/WORLD.h*mh; mctx.fillRect(x-1,y-1,2,2); } mctx.fillStyle='#e0e0e0'; mctx.fillRect(player.x/WORLD.w*mw-2, player.y/WORLD.h*mh-2, 4,4); mctx.strokeStyle='#9eb1ff'; mctx.lineWidth=1; mctx.strokeRect(cam.x/WORLD.w*mw, cam.y/WORLD.h*mh, W/WORLD.w*mw, H/WORLD.h*mh); }
